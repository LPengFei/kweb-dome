...
<c:forEach items="${page.list }" var="r" varStatus="s">
    <tr class="common_detail">
        ...
        <c:forEach items="${fields }" var="f">
            <c:if test="${f.field_name != 'picture' && f.list_view =='true' }">
                <kval:val model="${r }" field="${f }"></kval:val>
            </c:if>
            <c:if test="${f.field_name == 'picture' && f.list_view =='true' }">
                <td style="text-align: center">
                <c:if test="${not empty(r.picture)}">
                    <c:forEach items="${r.picture}" var="p" varStatus="vs">
                        <c:if test="${vs.count==1}">
                            <img class="icon_img fancy_arr" style="margin-right: 5px;" src="${ctx}/images/icon_img_active.png"
                                 alt="" height="25px" data-fancybox-group="button_sj_dgdw_person_${r.id}"
                                 href="${ctx }/kconf/file/downfile?type=pictures&filename=${p}">
                        </c:if>
                        <c:if test="${vs.count!=1}">
                            <img class="icon_img fancy_arr" style="display: none"
                                 src="${ctx}/images/icon_img_active.png" alt="" height="30px"
                                 data-fancybox-group="button_sj_dgdw_person_${r.id}"
                                 href="${ctx }/kconf/file/downfile?type=pictures&filename=${p}">
                        </c:if>
                    </c:forEach>
                </c:if>
