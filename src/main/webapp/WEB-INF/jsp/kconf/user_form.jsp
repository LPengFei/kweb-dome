					...
					<c:if test="${f.form_view_type ne 'hidden'}">
						...
						<li class="input_${f.form_view_type }" style="${f.form_style == "clear:left" ? "clear:left":fn:contains(f.form_style,'710px')?'width:750px;':''}">
							...
							<c:if test="${f.form_view_type eq 'file' }">
								<div id="doc_span_pic" data-toggle="upload" data-uploader="${ctx }/kconf/file/uploadfile?folder=sign&rename=${f.file_rename}"
...
<script type="text/javascript">
    ...
    function attachement_upload_success(file, data,element,input_id) {
        ...
        if (json[BJUI.keys.statusCode] == BJUI.statusCode.ok) {
            var exist_files = $("#"+input_id).val();
            $('#'+input_id).val(exist_files+json.filename);
            //console.debug("文件名称："+$("#doc_pic").val());
