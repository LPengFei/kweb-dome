...
//    $(function () {
        ...
        $.bindChecked('#'+s+t, function (data) {
            ...
            if(s=='work_dept') {
                $("input[name='record.work_dept_id']").val(function () {
                    if (data.length > 0) {
                        var arr = [];
                        $.each(data, function (k, v) {
                            arr.push(v.value.id)
                ...
                $("input[name='record.work_dept']").val(function () {
                    if (data.length > 0) {
                        var arr = [];
                        $.each(data, function (k, v) {
                            arr.push(v.value.dname)
                        });
                        return arr.join(',')
                    }
                });
            ...
            //管理人员
            if(s=='manager_dept_name'){
                $("input[name='record.manager_dept_id']").val(function () {
                    if (data.length > 0) {
                        var arr = [];
                        $.each(data, function (k, v) {
                            arr.push(v.value.id)
            ...
            //在编人员
            if(s=='dname'){
                $("input[name='record.dept_id']").val(function () {
                    if (data.length > 0) {
                        var arr = [];
                        $.each(data, function (k, v) {
                            arr.push(v.value.id)
                ...
                $("input[name='dept_id']",$.CurrentNavtab).val(function () {
                    if (data.length > 0) {
                        var arr = [];
                        $.each(data, function (k, v) {
                            arr.push(v.value.id)
            ...
            if(s='verify_dept_name'){
                $("input[name='verify_dept_id']").val(function () {
                    if (data.length > 0) {
                        var arr = [];
                        $.each(data, function (k, v) {
                            arr.push(v.value.id)
                ...
                $("input[name='record.verify_dept_id']").val(function () {
                    if (data.length > 0) {
                        var arr = [];
                        $.each(data, function (k, v) {
                            arr.push(v.value.id)
            ...
            $('#j_input_audit_deptname').val(data[0].value.dname);
            $("input[name='record.audit_deptid']").val(data[0].value.id)
...
<div style="height: 420px" id="${lookup_param}${lookup_modelId}">
    <div class="bjui-pageHeader" style=";background-color: white;margin: 0;padding: 0;border: none">
        <div class="dialog_search" style="margin-top: 0">
            <form id="pagerForm" data-toggle="ajaxsearch" action="${ctx}/kconf/${modelName}/${modelmethod}" method="post" style="margin: 0;padding: 0">
                ...
                <div class="ess_line_98 dialog_display" style="height: auto;">
    ...
    <div class="bjui-pageContent tableContent white ess-pageContent"
         style="width:100%;border: none;margin: 0;padding: 0;top: 75px;background-color: white">
                                ...
                                <c:if test="${chk_style}">
                                    <span class="input-wrap" >
                                        <input type="checkbox"
                                               id="${r.id}"
                                               name="result"
                                               content="<c:forEach items="${fields }" var="f" varStatus="s"><c:if test="${f.lookup_name =='true' }">${r[f.field_name]}</c:if></c:forEach>"><span
