...
$.fn.xb_scroll = function(options)
{
	...
	var scroll_config={
		...
		"barWidth":4,
		"childPanel":".active",
		"boxClass":"_scroll_box _scroll",
		"barClass":"_scroll_bar _scroll",

		"bgColor":" rgba(15,255,252,0.1)",
		"focus_bgColor":" rgba(15,255,252,0.3)"
	}
	...
	var jqScrollBar= $("<div style='position:absolute;width:"+opts.barWidth+"px;top:"+opts.barOffTop+"px;right:"+opts.barOffRight+"px;border-radius: 5px;background:"+opts.bgColor+";'></div>");
	...
	jqScrollBar.on("mousedown",function(ev){
		...
		jqScrollBar.css({"background":opts.focus_bgColor});
	...
	$(document).on("mouseup",function(ev){	
		...
		jqScrollBar.css({"background":opts.bgColor});
		isS_B = false;	
	});
	...
	function fnContentResize()								/*容器内元素变动更新滚动*/
	{
		...
		console.log(elem.innerHeight(),elChild.outerHeight());
		if(iRate >= 1){
			jqScrollBox.hide();
			jqScrollBar.css("height",0);
			elChild.css("top",0);	
		...
		iScrollBoxHeight = jqScrollBox.outerHeight();	//盒子高度
