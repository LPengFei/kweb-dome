    ...
    Upload.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            renderQueueItem: function(file) {
                ...
                //取消、删除上传文件
                $temp.on('click.bjui.upload.cancel', '.up_cancel', function(e){
                    var file_name = file.name;
                    var doc_pic_obj =  $('#'+options.inputid);
                    var new_file_names = doc_pic_obj.val();

                    new_file_names = new_file_names.replace(file_name + ',', '');
                    doc_pic_obj.val(new_file_names);
                    console.debug(doc_pic_obj.val());
            ...
            successQueueItem: function(file, xhr) {
                if(options.onUploadSuccess){
                    setTimeout(function(){
                      $(that.$element).find('#'+file.id).find('.percent').html('100%')
                      $(that.$element).find('#'+file.id).find('.up_filename').html($.parseJSON(xhr.responseText).filename.replace(/\,$/,''))
                    },200)
                };
                tools.showProgress(file.id, file.size, file.size);
                options.onUploadSuccess && options.onUploadSuccess.toFunc().call(that, file, xhr.responseText, that.$element,options.inputid)
                //在指定的间隔时间后删掉进度条
//                setTimeout(function() {
...
//                    if (filelen == 0) that.$element.find('> .queue').hide()
               ...
               // if(options.onCancel) options.onCancel.toFunc().call();
                e.preventDefault()
            },
            //预览上传图片
    ...
    Upload.prototype.init = function() {
        ...
        var $uploadFrag = $(FRAG.uploadFrag
            ...
            .replaceAll('#accept#', options.fileTypeExts)
        ...
        //编辑时为已存在的图片监听删除事件
        that.$element.find('.info > .up_cancel')
        .off('click.bjui.upload.cancel')
        .on('click.bjui.upload.cancel', $.proxy(function(e) {
        	var input_id = e.currentTarget.attributes['data-file-id'].value;
        	var file_name = e.currentTarget.attributes['data-file-name'].value;
        	var doc_pic_obj =  $('#'+input_id);
        	var new_file_names = doc_pic_obj.val();
        	new_file_names = new_file_names.replace(file_name+",",'');
        	doc_pic_obj.val(new_file_names);
        	console.debug(doc_pic_obj.val());
