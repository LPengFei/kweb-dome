...
public class ShiroPlugin implements IPlugin {
	...
	private static final Class<? extends Annotation>[] AUTHZ_ANNOTATION_CLASSES = new Class[] {
			...
			RequiresAuthentication.class, KRequiresPermissions.class };
	...
	public boolean start() {
		...
		ShiroKit.init(authzMaps, Authority.me.findAuthority());
		...
		if (PropKit.use("dbconfig.txt").getBoolean("devMode"))
			insertToMenu();
	...
	private void scanAnnotation(List<AuthzHandler> authzArray, List<Annotation> annotations) {
		...
		for (Annotation a : annotations) {
			...
			} else if (a instanceof KRequiresPermissions) {
				authzArray.set(5, new PermissionAuthzHandler(a));
			}
		}
	}
	...
	public void insertToMenu() {
		...
		for (Entry<String, Class<? extends Controller>> entry : routes.getEntrySet()) {
			...
			// 获取Controller的KRequiresPermissions注解。
			Annotation a = controllerClass.getAnnotation(KRequiresPermissions.class);
			Class modelClass = null;

			Authority pAuthority = null;
			if (a != null) {
				String aname = ((KRequiresPermissions) a).name();
				modelClass = ((KRequiresPermissions) a).model();
				//Menu.me.insertMenu(aname, controllerKey); // 去除菜单自动生成 by lihf
				pAuthority = Authority.me.insert(aname, controllerKey, null);
			}

			// 逐个遍历方法。
			Method[] methods = controllerClass.getMethods();
			for (Method method : methods) {
				Annotation m = method.getAnnotation(KRequiresPermissions.class);
				String actionKey = controllerKey;
				if (m != null) {
					String mname = ((KRequiresPermissions) m).name();
					if (!method.getName().equals("index")) {
						actionKey = controllerKey + "/" + method.getName();
					}
					if (pAuthority != null) {
						Authority.me.insert(mname, actionKey, pAuthority.get("aid"));
					}

					// 如果不包含这2个方法就跳过
					if (!(method.getName().equals("delete") || method.getName().equals("edit"))) {
						continue;
					}
				}
				...
				KModel kmodel = KModel.me.findByTableName(table.getName());
				if (kmodel == null) {
					continue;
				}

				if (method.getName().contains("delete")) {
					ModelLink link = ModelLink.me.findByPropertity(new String[] { "mid", "label" },
							new Object[] { kmodel.get("mid"), "删除" }, Logical.AND);
					if (link == null) {
						link = new ModelLink();
						link.set(link.getPkName(), -100);
						link.set("mid", kmodel.get("mid"));
						link.set("label", "删除");
						link.set("link",
								controllerKey + "/" + method.getName() + "?id={" + table.getPrimaryKey()[0] + "}");
						link.set("other",
								" class=\"red row-del\" data-toggle=\"doajax\" data-confirm-msg=\"确定要删除该行信息吗？\"");
						link.save();
					}
				}

				if (method.getName().contains("edit")) {
					ModelLink link = ModelLink.me.findByPropertity(new String[] { "mid", "label" },
							new Object[] { kmodel.get("mid"), "修改" }, Logical.AND);
					if (link == null) {
						link = new ModelLink();
						link.set(link.getPkName(), -100);
						link.set("mid", kmodel.get("mid"));
						...
						link.set("link",
								controllerKey + "/" + method.getName() + "?id={" + table.getPrimaryKey()[0] + "}");
						link.set("other",
								"data-toggle=navtab data-id={" + table.getPrimaryKey()[0] + "}-edit data-title=编辑");
						link.save();
					}
				}
